-- ============================================================
-- BRDC Employee Portal — Clean Reset + Full Migration
-- Target project : ryzucipineonaytxotuu
--
-- HOW TO RUN (one paste, one click):
--   1. Go to https://supabase.com/dashboard/project/ryzucipineonaytxotuu/sql/new
--   2. Select ALL text in the editor (Ctrl+A)
--   3. Paste this entire file (Ctrl+V)
...
DROP TABLE IF EXISTS broadcast_alert CASCADE;
DROP TABLE IF EXISTS support_requests CASCADE;
DROP TABLE IF EXISTS dept_announcements CASCADE;
DROP TABLE IF EXISTS admin_memos CASCADE;
...
DROP SEQUENCE IF EXISTS dept_announcements_id_seq;
DROP SEQUENCE IF EXISTS announcement_acknowledgements_id_seq;
...
CREATE SEQUENCE dept_announcements_id_seq;
CREATE SEQUENCE announcement_acknowledgements_id_seq;
CREATE SEQUENCE login_logs_id_seq;
...
-- profiles (32 rows)
INSERT INTO profiles (id, username, full_name, department, password, security_question, security_answer, is_admin, created_at, position) VALUES
...
INSERT INTO announcement_acknowledgements (id, announcement_id, username, full_name, dept, position, acknowledged_at) VALUES
...
--   Storage > New bucket > Name: MEMOS > Public: ON
-- ============================================================
